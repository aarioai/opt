...
readonly K8S_ALIYUN_HOST='registry.cn-hangzhou.aliyuncs.com'
...
k8sCheckTLSRefresh(){
    ...
    local _k8s_old_content=$(cat "${_k8s_dir}/content.txt")
    local _k8s_old_expire=$(cat "${_k8s_dir}/expire.txt")
    local _k8s_old_expire_days=$(echo "$_k8s_old_expire" | grep -o '[0-9]\+ days' | grep -o '[0-9]\+' | head -1)
    if [ "$_k8s_content" != "$_k8s_old_content" ] || [ "$_k8s_expire_days" != "$_k8s_old_expire_days" ]; then
        return 0
    fi

    local _k8s_t1=$(date -d "$_k8s_old_expire" +%s)
    local _k8s_t2=$(date -d '+ 30 days' +%s)        # 最后30天内，都需要更新
